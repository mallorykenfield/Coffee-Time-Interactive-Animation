...
export class WhippedCream {
    ...
    display(context, program_state, model_transform) {
        ...
        if (this.frame > p3_end) {
            ...
            let offset = Mat4.translation(x - 25, 0, -1 * y);
            whipped_base_transform = offset.times(Mat4.rotation(Math.PI/2 * r, 0, -1, -1)).times(whipped_base_transform);
        ...
        // phase 3
            ...
            let x = Math.cos(r/2) * b_y;
            let y = Math.sin(r/2) * b_y;
            let offset = Mat4.translation(x - 25, 10, -1 * y);
            whipped_base_transform =  offset.times(Mat4.rotation(Math.PI/2 * r, 0, -1, -1)).times(whipped_base_transform);
